...
export class BuscarPortafoliosComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    // Inicializar formulario de filtros extendido
    this.filtrosForm = this.fb.group({
      ...
      // Nuevos filtros
      tipoPerfil: [[]],  // Array de tipos seleccionados
      promedioMin: [0],
      salarioMin: [''],
      salarioMax: [''],
      ratingMin: [0],
      modalidad: [''],
      anioEstudio: [''],
      idiomas: [[]],  // Array de idiomas seleccionados
      experiencia: ['']
    });
  ...
  calcularFiltrosActivos(): void {
    ...
    // Tipo de perfil
    if (formValue.tipoPerfil && formValue.tipoPerfil.length > 0) {
      formValue.tipoPerfil.forEach((tipo: string) => {
        const option = this.tiposPerfilOptions.find(o => o.value === tipo);
        this.filtrosActivos.push({
          key: 'tipoPerfil',
          label: `Tipo: ${option?.label || tipo}`,
          value: tipo
        });
      });
    }
  ...
  removerFiltro(filtro: FiltroActivo): void {
    if (filtro.key === 'searchText') {
      this.searchText = '';
    } else if (filtro.key === 'tipoPerfil') {
      const tipos = this.filtrosForm.get('tipoPerfil')?.value || [];
      const index = tipos.indexOf(filtro.value);
      if (index > -1) {
        tipos.splice(index, 1);
        this.filtrosForm.patchValue({ tipoPerfil: tipos });
      }
    } else if (filtro.key === 'idiomas') {
      const idiomas = this.filtrosForm.get('idiomas')?.value || [];
      const index = idiomas.indexOf(filtro.value);
      if (index > -1) {
        idiomas.splice(index, 1);
        this.filtrosForm.patchValue({ idiomas: idiomas });
      }
    } else if (filtro.key === 'salario') {
      this.filtrosForm.patchValue({ salarioMin: '', salarioMax: '' });
    } else {
      // Para el resto, resetear el campo individual
      const resetValue = (filtro.key === 'promedioMin' || filtro.key === 'ratingMin') ? 0 : '';
      this.filtrosForm.patchValue({ [filtro.key]: resetValue });
    }
  ...
  // === Manejo de checkboxes de tipo de perfil ===

  onTipoPerfilChange(event: any, tipo: string): void {
    const tipos = this.filtrosForm.get('tipoPerfil')?.value || [];
  ...
  isTipoPerfilSelected(tipo: string): boolean {
    const tipos = this.filtrosForm.get('tipoPerfil')?.value || [];
  ...
  buscar(pagina: number = 1): void {
    ...
    // Texto de búsqueda
    if (this.searchText.trim()) {
      filtros.texto = this.searchText.trim();
    }

    // Tipo de perfil
    if (formValue.tipoPerfil && formValue.tipoPerfil.length > 0) {
      filtros.tipoPerfil = formValue.tipoPerfil;
    }
