...
export class OpportunityListComponent implements OnInit {
  ...
  editar(opp: Opportunity): void {
    const id = opp.id || opp.id_oportunidad;
  ...
  pause(opp: Opportunity): void {
    ...
    this.service.changeState(id, 'PAUSADA').subscribe({
      next: () => this.load(),
      error: (err) => console.error('Error al pausar', err),
    });
  }
  ...
  reanudar(opp: Opportunity): void {
    if (opp.estado === 'PAUSADA') {
      ...
      this.service.changeState(id, 'ACTIVA').subscribe({
        next: () => this.load(),
        error: (err) => console.error('Error al reanudar', err),
      });
    }
  }
  ...
  duplicar(opp: Opportunity): void {
    ...
    this.service.duplicate(id).subscribe({
      next: () => this.load(),
      error: (err) => console.error('Error al duplicar', err),
    });
  }
